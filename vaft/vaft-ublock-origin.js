(function() {
    ...
    // FIX 2: Safe string escaping for worker blob header injection
    function safeStr(v) {
        ...
        const newWorker = class Worker extends getCleanWorker(window.Worker) {
            constructor(twitchBlobUrl, options) {
                ...
                const newBlobStr = `
                    ...
                    declareOptions(self);
                    GQLDeviceID = ${safeStr(GQLDeviceID)};
                    AuthorizationHeader = ${safeStr(AuthorizationHeader)};
                    ClientIntegrityHeader = ${safeStr(ClientIntegrityHeader)};
                    ClientVersion = ${safeStr(ClientVersion)};
                    ClientSession = ${safeStr(ClientSession)};
    ...
    function stripAdSegments(textStr, stripAllSegments, streamInfo) {
        ...
        for (let i = 0; i < lines.length; i++) {
            let line = lines[i];
            // FIX 1: Use new RegExp instances per iteration to avoid /g lastIndex bugs
            lines[i] = line
                .replace(new RegExp(RE_AD_URL.source, 'g'), `$1${newAdUrl}$2`)
                .replace(new RegExp(RE_AD_CLICK_URL.source, 'g'), `$1${newAdUrl}$2`);
    ...
    async function processM3U8(url, textStr, realFetch) {
        // FIX 4: Purge stale AdSegmentCache on every call, not just during ad stripping
        const now = Date.now();
        AdSegmentCache.forEach((value, key, map) => {
            if (value < now - 120000) map.delete(key);
        });
